...
class ChangesView(View):
    ...
    def get(self, request):
        ...
        def get_data(from_date, to_date, per_page, page):
            ...
            edits = LatestResourceEdit.objects.filter(
                timestamp__range=(from_date, to_date)
            ).order_by("timestamp")

            # Remove settings changes
            if settings.SYSTEM_SETTINGS_RESOURCE_ID in [
                edit.resourceinstanceid for edit in edits
            ]:
                edits = edits.exclude(
        ...
        permissions = UserObjectPermission.objects.filter(
        ...
        filtered_data = [
            latest_edit
            for latest_edit in db_data[0]
            if latest_edit.resourceinstanceid not in no_access_list
        ]
