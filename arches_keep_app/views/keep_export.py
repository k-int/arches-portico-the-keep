    ...
    if request.method == "POST":
        ...
        for resource_id in resource_ids:
            ...
            try:
                ...
                if str(resource.graph_id) not in [
                    monument_graph_id,
                    artifact_graph_id,
                    area_graph_id,
                ]:
                ...
                if str(resource.graph_id) == monument_graph_id:  # monument exclusions
                    for tile in resource.tiles:
                        if (
                            ...
                            == "6af2a0cb-efc5-11eb-8436-a87eeabdefba"
                        ...
                        if (
                            str(tile.nodegroup_id)
                            == "055b3e3f-04c7-11eb-8d64-f875a44e0e11"
                        ):  # Associated Monuments, Areas or Artefacts
                            if tile.data["055b3e44-04c7-11eb-b131-f875a44e0e11"]:
                                assoc_monument_ids = [
                                    assoc_resource["resourceId"]
                                    for assoc_resource in tile.data[
                                ...
                                for id in assoc_monument_ids:
                                    ...
                                    if (
                                        str(assoc_resource.graph_id)
                                        == "b8032b00-594d-11e9-9cf0-18cf5eb368c4"
                                    ):  # aircraft monument
                                        exclude_flag = True
                                    if (
                                        str(assoc_resource.graph_id)
                ...
                if str(resource.graph_id) == area_graph_id:  # area exclusions
                    for tile in resource.tiles:
                        if (
                            str(tile.nodegroup_id)
                            == "a4a81528-efa9-11eb-9abd-a87eeabdefba"
                        ):  # designation and protection assignment
                            exclude_flag = True

                        if (
                            str(tile.nodegroup_id)
                            == "d17a5389-28cd-11eb-9670-f875a44e0e11"
                        ):  # area assignment
                            exclude_flag = True
                ...
                if str(resource.graph_id) == artifact_graph_id:  # artefact exclusions
                    for tile in resource.tiles:
                        if (
                            str(tile.nodegroup_id)
                            == artifact_node_ids["system_refs_id"]
                        ):
                            legacy_id = tile.data[artifact_node_ids["legacy_id"]]

                            if legacy_id:

                                legacy_MES_id_match = re.search(
                                    r"\bMES\d+\b", legacy_id["en"]["value"]
                                )
                                legacy_MES_id = (
                                    legacy_MES_id_match.group()
                                    if legacy_MES_id_match
                                    else None
                                )

                                if (
                                    not legacy_MES_id
                                ):  # only artefacts with MES in legacy id
                                    exclude_flag = True
                                elif (
                                    legacy_MES_id in included_findspots
                                ):  # only included once
                                    exclude_flag = True
                                else:
                ...
                if not exclude_flag:
                    ...
                    system_refs_tile = Tile.objects.filter(
                        resourceinstance_id=resource.resourceinstanceid,
                        nodegroup_id=id_lookup["system_refs_id"],
                    )

                    if len(system_refs_tile) == 1:

                        if system_refs_tile[0].data[id_lookup["primary_ref_id"]]:
                            primary_id = system_refs_tile[0].data[
                                id_lookup["primary_ref_id"]
                            ]

                        if system_refs_tile[0].data[id_lookup["legacy_id"]]:
                            legacy_id = system_refs_tile[0].data[
                                id_lookup["legacy_id"]
                            ]["en"]["value"]
                    ...
                    for tile in resource.tiles:  # retrieve id variables
                        ...
                        if str(tile.nodegroup_id) == id_lookup["names_id"]:  # names
                            monument_name = tile.data[id_lookup["name_id"]]["en"][
                                "value"
                            ]
                            monument_name_type = tile.data[id_lookup["name_type_id"]]
                            mon_name_obj = {
                                "mon_name": monument_name,
                                "mon_name_type": monument_name_type,
                            }
                            mon_names.append(mon_name_obj)

                        if (
                            str(tile.nodegroup_id) == id_lookup["descriptions_id"]
                        ):  # descriptions

                            if (
                                tile.data[id_lookup["description_type_id"]]
                                == "35508b82-062a-469f-830a-6040c5e5eb8c"
                            ):  # summary type
                                summary = tile.data[id_lookup["description_id"]]["en"][
                                    "value"
                                ]
                            ...
                            if (
                                tile.data[id_lookup["description_type_id"]]
                                == "39a21ebf-7dd6-4a7f-a211-9453202f60aa"
                            ):  # full type
                                full_description = tile.data[
                                    id_lookup["description_id"]
                                ]["en"]["value"]
                        ...
                        if (
                            ...
                            if (
                                str(tile.nodegroup_id) == id_lookup["record_type_id"]
                            ):  # resource model type
                                model_type = tile.data[id_lookup["record_type_id"]]
                                model_type_label = (
                                    "Building"
                                    if model_type
                                    == "5e5d6f01-fcd9-4ba0-b86d-564456a520b2"
                                    else "Monument"
                                )
                        ...
                        if (
                            str(tile.nodegroup_id) == id_lookup["national_grid_refs_id"]
                        ):  # grid reference
                        ...
                        if str(tile.nodegroup_id) == id_lookup["admin_areas_id"]:
                            ...
                            area_type_value_string = (
                                "Civil Parish"
                                if area_type_value.value == "Parish"
                                else area_type_value.value
                            )

                            area_name = tile.data[
                                id_lookup["area_name_id"]
                        ...
                        if (
                            str(tile.nodegroup_id)
                            == id_lookup["construction_phases_id"]
                        ):  # find construction phase
                            ...
                            if str(resource.graph_id) in [
                                "076f9381-7b00-11e9-8d6b-80000b44d1d9",
                                "979aaf0b-7042-11ea-9674-287fcf6a5e72",
                            ]:  # monument or area

                                monument_types = tile.data[
                                    id_lookup["monument_types_id"]
                                ]  # monument type
                                ...
                                type_certainty = tile.data[
                                    id_lookup["type_certainty_id"]
                                ]  # type confidence
                                if (
                                    type_certainty
                                    == "2d32062f-80b4-4293-94aa-46653ba5c632"
                                ):
                            ...
                            monument_type_object["UnknownDate"] = (
                                0 if date_start and date_end else 1
                            )

                            date_certainty = tile.data[id_lookup["date_certainty_id"]]
                            date_certainty_value = (
                                "?"
                                if date_certainty
                                == "2d32062f-80b4-4293-94aa-46653ba5c632"
                                else ""
                            ...
                            if date_qualifier:
                                ...
                                monument_type_object["DateQualifier"] = (
                                    date_qualifier_value.value
                                )
                        ...
                        if str(resource.graph_id) in [
                            ...
                            if (
                                str(tile.nodegroup_id)
                                == "55d6a53e-049c-11eb-8618-f875a44e0e11"
                            ):  # find components
                                ...
                                component_types = tile.data[
                                    "46cd4b7e-049d-11eb-ba3a-f875a44e0e11"
                                ]
                                component_types_list = []
                                for component_type in component_types:
                                    component_value = Value.objects.get(
                                        valueid=component_type
                                    )
                                    component_types_list.append(
                                        component_value.value.upper()
                                    )
                                components_string = "; ".join(component_types_list)

                                component_obj["MonType"] = components_string

                                construction_phase_tileid = tile.data[
                                    "a0c7f934-04a4-11eb-9d78-f875a44e0e11"
                                ]

                                # find the associated construction phase already stored on the data object
                                if construction_phase_tileid:
                                    mon_type_tile = Tile.objects.get(
                                        pk=construction_phase_tileid
                                    )

                                    date_start = mon_type_tile.data[
                                        id_lookup["date_start_id"]
                                    ]
                                    if isinstance(date_start, str):
                                        date_start = date_start.replace("y-", "-")
                                    component_obj["FromDate"] = date_start

                                    date_end = mon_type_tile.data[
                                        id_lookup["date_end_id"]
                                    ]
                                    if isinstance(date_end, str):
                                        date_end = date_end.replace("y-", "-")
                    ...
                    if len(mon_names) > 0:
                        ...
                        if (
                            str(resource.graph_id) == artifact_graph_id
                        ):  # for artefacts, separate primary and alternative names, and take first alternative if exists
                            primary_names = [
                                name
                                for name in mon_names
                                if name["mon_name_type"]
                                == "2df285fa-9cf2-45e7-bc05-a67b7d7ddc2f"
                            ]
                            alternative_names = [
                                name
                                for name in mon_names
                                if name["mon_name_type"]
                                == "e9170ee3-3455-4ec3-b596-475c0969f3bf"
                            ]
                    ...
                    if len(mon_descriptions) > 0:
                        if (
                            str(resource.graph_id) == artifact_graph_id
                        ):  # for artefacts, choose last full description
                            mon_object["Description"] = mon_descriptions[-1]
                        else:
                            mon_object["Description"] = max(
                                mon_descriptions, key=len
                            )  # otherwise choose longest description
        ...
        if len(resources_with_no_pid) > 0:
            warnings.warn(
                f"Warning, the following resources are missing a primary_id: {resources_with_no_pid}"
            )
